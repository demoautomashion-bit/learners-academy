...
export default function FeeRegistryPage() {
  ...
  const exportFeeRegistryPDF = () => {
    try {
        ...
        doc.setFontSize(14)
        doc.setFont('helvetica', 'bold')
        doc.text(`FEE AUDIT REPORT - ${activeSeason.toUpperCase()} TERM`, 15, 50)
        
        doc.setFontSize(8)
        doc.setFont('helvetica', 'normal')
        doc.text(`Generation Date: ${new Date().toLocaleString()}`, 15, 56)

        // 3. Financial Summary Matrix (Aggregated from current filtered view)
        const totalPaid = studentLedger.reduce((acc, s) => acc + s.amountPaid, 0)
        const totalDues = studentLedger.reduce((acc, s) => acc + s.amountRemaining, 0)
        const totalDiscount = studentLedger.reduce((acc, s) => acc + s.discountGiven, 0)

        const summaryY = 65
        const stats = [
            { label: 'Total Collected', value: `PKR ${totalPaid.toLocaleString()}`, color: [34, 197, 94] },
            { label: 'Total Dues', value: `PKR ${totalDues.toLocaleString()}`, color: [239, 68, 68] },
            { label: 'Discounts Issued', value: `PKR ${totalDiscount.toLocaleString()}`, color: [31, 41, 55] }
        ]
        ...
        // 4. Student Ledger Table
        const tableData = studentLedger.map((s) => [
            `${s.name}\n(${s.studentId})`,
            s.classTitle,
            { 
                content: `PKR ${s.amountPaid.toLocaleString()}`,
                styles: { textColor: [34, 197, 94], fontStyle: 'bold' }
            },
            { 
                content: `PKR ${s.amountRemaining.toLocaleString()}`,
                styles: { textColor: s.amountRemaining > 0 ? [239, 68, 68] : [100, 100, 100] }
            }
        ])

        autoTable(doc, {
            startY: 95,
            head: [['Student Info', 'Class/Grade', 'Paid Amount', 'Dues Remaining']],
            body: tableData,
            theme: 'striped',
        ...
        const footerY = Math.max(finalY + 25, 250)
        ...
        doc.setFontSize(7)
        doc.text("Accounts Officer", 15, footerY + 5)
        doc.text("Institutional Seal", pageWidth - 15, footerY + 5, { align: 'right' })
        
        doc.setFontSize(6)
        doc.text("Official Registry Extract - Generated by Nexilumina Solutions Audit Engine", 15, footerY + 15)
        doc.text(`Ref: TLA-FEE-${new Date().getTime().toString().slice(-6)}`, pageWidth - 15, footerY + 15, { align: 'right' })

        // Save
        const dateStamp = new Date().toISOString().split('T')[0]
        doc.save(`TLA_Fee_Registry_${activeSeason}_${dateStamp}.pdf`)
        toast.success("Registry Exported")
        
    } catch (error) {
        console.error("FEE_PDF_CRITICAL:", error)
