...
export default function BroadcastPage() {
  ...
  const handleSendBroadcast = async () => {
    ...
    toast.success(`Broadcast sent! ${count} recipients notified.`)
    setSubject('')
    setIsSending(false)
  }
          ...
          <Card className="glass-1 rounded-[2rem] border-primary/5 shadow-2xl p-8 space-y-6">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <Input value={subject} onChange={e => setSubject(e.target.value)} placeholder="e.g. Term 2 General Registration Open" className="h-12 bg-primary/[0.02] border-none" />
                </div>
                <div className="space-y-2">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex gap-2">
                    {['{Name}', '{Term}', '{UnsubscribeLink}'].map(tag => (
                      <button key={tag} onClick={() => handleInsertTag(tag)}
                ...
                <Textarea value={message} onChange={e => setMessage(e.target.value)} rows={6}
                  className="bg-primary/[0.02] border-none resize-none leading-relaxed" placeholder="Draft message body..." />
              </div>
            ...
            <div className="flex justify-end pt-4">
              <Button onClick={handleSendBroadcast} disabled={isSending}
                ...
                {isSending ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Sending...</> : <><Send className="w-4 h-4 mr-2" />Send Text Message</>}
              </Button>
            </div>
          </Card>
        </div>
